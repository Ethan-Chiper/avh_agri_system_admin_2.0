	...
	this.signUp = async (req, res) => {
		try {
			...
			let farmerData = {
				...
				auth_token: {
					'web': data?.web, 'mobile': data?.mobile
				},
				...
				settings: {
					...
					time_zone: {
						'name': 'Asia/ Kolkata', 'label': 'Indian Standard Time', 'offset': '+0530'
					},
					branding: {
						'theme_color': '', 'text_color': '', 'icon': ''
					},
			...
			let createData = await FarmerModel.create(farmerData);
				if (createData) {
					...
					await Utils.createUserAndTokenInKong(userId, (token) => {
						...
						if (token) return Responder.sendSuccessData(res, ' account created successfully', {
